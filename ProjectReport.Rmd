...
n<-c(2,5,10,30); fitaccs<-array(data=NA,dim=c(length(n),4))  ## Store the fit accuracies for models with up to n cross-validations
for (i in 1:length(n)) {
  set.seed(12345)
...
With just 2 cross-validations, our model fit completes in just 2-4 minutes of total computation time with a resampling accuracy of 96.3%. Applying this model achieves 19/20 correct test predictions, or approximately 95% of the test cases are predicted accurately, as we might have expected given our estimated resampling accuracy. Therefore, we might do slightly better by increasing the number of cross-validations performed (at the expense of computation time). 
